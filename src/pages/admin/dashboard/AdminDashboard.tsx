...
export default function AdminDashboard() {
  ...
  return <div className="mx-auto w-full max-w-[1440px] space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="hp-card overflow-hidden rounded-2xl"><div className="p-5 pb-3"><SectionHeader title="Pending / Recent KYC" description="Retailers requiring administrative attention" action="Review KYC" onAction={() => navigate("/admin/retailers")} /></div><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead className="border-y border-slate-100 bg-slate-50/70 text-[10px] uppercase tracking-wider text-slate-400"><tr><th className="px-5 py-3 font-bold">Retailer ID</th><th className="px-5 py-3 font-bold">Retailer</th><th className="px-5 py-3 font-bold">Mobile</th><th className="px-5 py-3 font-bold">Registered</th><th className="px-5 py-3 font-bold">Status</th><th className="px-5 py-3 font-bold">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{recentRetailers.map((retailer) => <tr key={retailer.id} className="text-slate-600"><td className="px-5 py-3 font-semibold text-[#315bd1]">{retailer.id}</td><td className="px-5 py-3 font-semibold text-slate-800">{retailer.name}</td><td className="px-5 py-3">{retailer.mobile}</td><td className="px-5 py-3">{retailer.registrationDate}</td><td className="px-5 py-3"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${statusClass[retailer.kycStatus]}`}>{retailer.kycStatus}</span></td><td className="px-5 py-3"><button type="button" onClick={() => navigate("/admin/retailers")} className="font-bold text-[#315bd1] hover:underline">View</button></td></tr>)}</tbody></table></div></div>
      <div className="hp-card rounded-2xl p-5"><SectionHeader title="Transaction Status" description="Quick view of current transaction outcomes" /><div className="space-y-4"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-sm text-slate-600"><CheckCircle2 className="h-4 w-4 text-emerald-500" />Success</span><span className="font-bold text-slate-900">72%</span></div><div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[72%] rounded-full bg-emerald-500" /></div><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-sm text-slate-600"><Clock3 className="h-4 w-4 text-amber-500" />Pending</span><span className="font-bold text-slate-900">18%</span></div><div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[18%] rounded-full bg-amber-500" /></div><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-sm text-slate-600"><XCircle className="h-4 w-4 text-rose-500" />Failed</span><span className="font-bold text-slate-900">10%</span></div><div className="h-2 rounded-full bg-slate-100"><div className="h-2 w-[10%] rounded-full bg-rose-500" /></div></div></div>
    </section>

    <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="hp-card overflow-hidden rounded-2xl"><div className="p-5 pb-3"><SectionHeader title="Recent Transactions" description="Latest AEPS, DMT and CMS activity" action="View Transactions" onAction={() => navigate("/admin/transactions")} /></div><div className="overflow-x-auto"><table className="w-full min-w-[690px] text-left text-xs"><thead className="border-y border-slate-100 bg-slate-50/70 text-[10px] uppercase tracking-wider text-slate-400"><tr><th className="px-5 py-3 font-bold">Transaction ID</th><th className="px-5 py-3 font-bold">Retailer</th><th className="px-5 py-3 font-bold">Service</th><th className="px-5 py-3 font-bold">Amount</th><th className="px-5 py-3 font-bold">Status</th><th className="px-5 py-3 font-bold">Date / Time</th></tr></thead><tbody className="divide-y divide-slate-100">{recentTransactions.map((transaction) => <tr key={transaction.id} className="text-slate-600"><td className="px-5 py-3 font-semibold text-[#315bd1]">{transaction.id}</td><td className="px-5 py-3 font-semibold text-slate-800">{transaction.retailer}</td><td className="px-5 py-3">{transaction.service}</td><td className="px-5 py-3 font-semibold text-slate-800">{money(transaction.amount)}</td><td className="px-5 py-3"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${statusClass[transaction.status]}`}>{transaction.status}</span></td><td className="px-5 py-3">{transaction.date}</td></tr>)}</tbody></table></div></div>
    ...
    <section className="grid gap-6 lg:grid-cols-3"><div className="hp-card rounded-2xl p-5"><SectionHeader title="CMS Configuration" description="Summary of platform configuration" /><div className="space-y-3 text-sm"><div className="flex justify-between"><span className="text-slate-500">CMS Companies</span><strong>{cmsSummary.companies}</strong></div><div className="flex justify-between"><span className="text-slate-500">Product Lines</span><strong>{cmsSummary.productLines}</strong></div><div className="flex justify-between"><span className="text-slate-500">Commission</span><strong className="text-emerald-600">{cmsSummary.commission}</strong></div><div className="flex justify-between"><span className="text-slate-500">Limits</span><strong className="text-emerald-600">{cmsSummary.limits}</strong></div></div><button type="button" onClick={() => navigate("/admin/cms/companies")} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#eef2ff] py-2.5 text-xs font-bold text-[#315bd1] hover:bg-[#e1e9ff]">Manage CMS<Settings2 className="h-3.5 w-3.5" /></button></div><div className="hp-card rounded-2xl p-5"><SectionHeader title="Customer Overview" description="Customers using AEPS and DMT" /><div className="grid grid-cols-3 gap-2 text-center"><div><p className="text-xl font-bold text-slate-900">{customerOverview.total.toLocaleString("en-IN")}</p><p className="mt-1 text-[10px] text-slate-500">Total</p></div><div><p className="text-xl font-bold text-[#315bd1]">{customerOverview.aeps.toLocaleString("en-IN")}</p><p className="mt-1 text-[10px] text-slate-500">AEPS</p></div><div><p className="text-xl font-bold text-emerald-600">{customerOverview.dmt.toLocaleString("en-IN")}</p><p className="mt-1 text-[10px] text-slate-500">DMT</p></div></div><button type="button" onClick={() => navigate("/admin/customers")} className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50">View Customers<Users className="h-3.5 w-3.5" /></button></div><div className="hp-card rounded-2xl p-5"><SectionHeader title="Recent Admin Activity" description="Administrative actions from the audit trail" action="View Audit Logs" onAction={() => navigate("/admin/audit-logs")} /><div className="space-y-4">{recentAdminActivity.map((activity) => <div key={`${activity.entity}-${activity.date}`} className="flex gap-3"><div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#eef2ff] text-[#315bd1]"><FileClock className="h-3.5 w-3.5" /></div><div className="min-w-0"><p className="text-xs font-bold text-slate-800">{activity.action}</p><p className="mt-0.5 text-[11px] text-slate-500">{activity.entity} · {activity.admin}</p><p className="mt-1 text-[10px] text-slate-400">{activity.date}</p></div></div>)}</div></div></section>
